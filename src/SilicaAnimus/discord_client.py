...
class MemberProcessView(discord.ui.View):
    ...
    async def button_get(
        ...
        member_info = await self.client.gsheet_client.get_member_by_discord_name(
        ...
        # if the member is in the sheet
            ...
            if await self.client.helloasso_client.get_membership(
                first_name=member_info.first_name, last_name=member_info.last_name
            ):
            ...
            # if the user is not member of the association this year
                ...
                embed.description = (
                    "Votre compte discord est associé au nom "
        ...
        # if the member is not in the sheet
            ...
            if await self.client.helloasso_client.get_membership(
                ...
                if len(identity_info.discord_nickname) > 0:
                    embed.description = (
                        "Ces nom / prénom sont déjà associés "
...
class MemberProcessModal(discord.ui.Modal, title="Devenir membre sur le discord"):
    """
...
class CheckModal(discord.ui.Modal, title="Informations"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        is_member = await ha_client.get_membership(
            first_name=prenom.value, last_name=nom.value
        )
        ...
        embed = MessageTemplate(
            title="Vérification du membre :", description=return_message
        )
...
class UpdateMemberButtons(discord.ui.View):
    ...
    @discord.ui.button(
        label="Afficher les membres masqués",
        style=discord.ButtonStyle.primary,
        disabled=True,
        custom_id="display",
    )
    async def button_display(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        pass
    ...
    async def button_confirm(
        ...
        for user in self.data["to_unmember"]:
            ...
            self.logger.info(f"{user} is removed from the member list")
...
async def give_role(
    ...
    for member in user_group.members:
        try:
            ...
            message += str(e)
            embed = MessageTemplate(
                title="Une erreur est survenue...", description=message
            )
            await interaction.followup.send(embed=embed)
            raise

        embed = MessageTemplate(title="Affectation de roles :", description=message)
...
async def info(interaction: discord.Interaction, member: discord.Member):
    ...
    member_info: MemberInfo = await parent.gsheet_client.get_member_by_discord_name(
        member.name
    )
...
async def update_member_list(interaction: discord.Interaction):
    ...
    embed = MessageTemplate(title=""" Mise à jour des adhérents sur le Discord""")

    # get the user list to member
    to_mem_str = ", ".join(
        [interaction.guild.get_member_named(member).mention for member in to_member]
    )
    ...
    # make message
    to_mem_str_short = ",".join(display_mem_l) + f"\n{max(0, hidden_member_members)}"
    ...
    to_unmem_str = ", ".join(
        [interaction.guild.get_member_named(member).mention for member in to_unmember]
    )
    ...
    # make message
    to_unmem_str_short = ",".join(display_unmem_l) + f"\n{max(0, hidden_unmem_members)}"
    ...
    to_keep_str = ", ".join(
        [interaction.guild.get_member_named(member).mention for member in to_keep]
    )
    ...
    # make message
    to_keep_str_short = ",".join(display_keep_l) + f"\n{max(0, hidden_keep_members)}"
    ...
    buttons = UpdateMemberButtons(
        logger=client.logger,
        embed=embed,
        role=role,
        data={"to_member": to_member, "to_unmember": to_unmember},
    )
...
class DiscordClient:
    ...
    def __init__(
        ...
        self.client = ThalosBot(command_prefix="!", intents=self.intents)
